...
export default function BackupSettingsPage() {
  ...
  const handleDownload = async () => {
    try {
      ...
      await download(downloadUrl, "mintax-backup.zip")
    } catch (error) {
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Download your data archive or restore from a previous backup.
        </p>
      </div>
      ...
      <Card className="flex flex-col gap-2 mt-16 p-5 bg-destructive/10 max-w-xl">
      ...
      {restoreState?.success && (
        <Card className="flex flex-col gap-2 p-5 bg-secondary max-w-xl">
