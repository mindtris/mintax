...
import { isFiltered, useHireFilters } from "@/lib/hooks/use-hire-filters"
import { ColumnsIcon, Filter, X, Search } from "lucide-react"
import { useState } from "react"

type ColumnConfig = { key: string; label: string }

export function HireSearchAndFilters({
  tab,
  columns = [],
  visibleColumns = [],
  onToggleColumn,
  ...
  const [filters, setFilters] = useHireFilters()
            ...
            <DropdownMenuContent align="end" className="w-48">
              {columns.map((col) => (
                <DropdownMenuCheckboxItem
                  ...
                  disabled={col.key === "title" || col.key === "name"}
                >
                  {col.label}
                </DropdownMenuCheckboxItem>
              ))}
