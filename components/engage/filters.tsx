...
import { isFiltered, useEngageFilters } from "@/lib/hooks/use-engage-filters"
import { ColumnsIcon, Filter, X, Search } from "lucide-react"
import { useState } from "react"

type ColumnConfig = { key: string; label: string }

export function EngageSearchAndFilters({
  columns = [],
  visibleColumns = [],
  onToggleColumn,
  categories = [],
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex gap-2">
        ...
        {isFiltered(filters) && (
          <Button variant="ghost" size="icon" onClick={() => setFilters({})} className="text-muted-foreground hover:text-foreground" title="Clear all filters">
            <X className="h-4 w-4" />
          </Button>
        )}
        ...
        <SheetContent side="right" className="inset-y-auto top-1/2 -translate-y-1/2 right-4 h-[96vh] w-[95vw] sm:max-w-md flex flex-col gap-0 p-0 overflow-hidden shadow-2xl">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select value={filters.status || "-"} onValueChange={(v) => handleFilterChange("status", v)}>
                <SelectTrigger className="w-full h-10"><SelectValue placeholder="All statuses" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={filters.provider || "-"} onValueChange={(v) => handleFilterChange("provider", v)}>
                <SelectTrigger className="w-full h-10"><SelectValue placeholder="All platforms" /></SelectTrigger>
                <SelectContent>
          ...
          <SheetFooter className="px-6 py-4 shrink-0 border-t">
            <div className="flex gap-2 w-full">
              ...
              <Button variant="outline" className="flex-1" onClick={() => { setFilters({}); setFilterSheetOpen(false) }}>Clear all</Button>
            </div>
          </SheetFooter>
        </SheetContent>
      </Sheet>
