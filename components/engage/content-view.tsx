      ...
      {templates.length > 0 ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {templates.map((template) => (
            <Card key={template.id} className="border border-border/50 shadow-sm shadow-black/[0.02] bg-card text-card-foreground rounded-2xl overflow-hidden hover:shadow-md transition-shadow cursor-pointer">
              <CardContent className="pt-6">
                <div className="font-semibold mb-1">{template.name}</div>
                {template.category && (
                  <Badge variant="outline" className="text-[10px] font-medium mb-2">{template.category}</Badge>
                )}
                <p className="text-sm text-card-foreground line-clamp-3">{template.content}</p>
